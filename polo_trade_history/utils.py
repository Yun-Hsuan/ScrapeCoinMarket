...
def Parser(pth):
    if not os.path.exists(pth):
        print("[ERROR][Parser] path not exists. ")
        exit(1)
    f = open(pth,"r")
    lines = f.readlines()
    Pair = []
    date_end,date_begin,period = None,None,None
    for line in lines:
        if '#' in line:
            continue
        elif '=' in line :
    ...
    ##checking:
    if date_end is None:
        print ("[ERROR] date_end not set.")
        exit(1)
    if date_begin is None:
        print ("[ERROR] date_begin not set.")
        exit(1)
    if period is None:
        print ("[ERRIR] period not set.")
        exit(1)
    return Pair, date_begin, date_end , period
